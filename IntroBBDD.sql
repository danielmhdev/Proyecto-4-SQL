...
SELECT city, COUNT(*) AS num_airports FROM airports_data GROUP BY city HAVING COUNT(*) >1;

/* 7. Mostrar el número de vuelos por modelo de avión. */
SELECT m.model, COUNT(*) AS num_vuelos FROM flights f JOIN aircrafts_data m ON f.aircraft_code = m.aircraft_code GROUP BY m.model;

/* 8. Reservas con más de un billete (varios pasajeros). */

SELECT book_ref, COUNT(*) AS num_tickets FROM tickets GROUP BY book_ref HAVING COUNT(*) >1; 
